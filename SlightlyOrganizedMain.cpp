...
std::tuple<unsigned int, unsigned int> setup2DTriangle() {
    float vertices[] {
            -0.5f, -0.5f, 0.0f,
            0.5f, -0.5f, 0.0f,
            0.0f, 0.5f, 0.0f
...
int main() {
    ...
    std::tuple<unsigned int, unsigned int> vbo_and_vao = setup2DTriangle();

    unsigned int vbo = std::get<0>(vbo_and_vao);
    unsigned int vao = std::get<1>(vbo_and_vao);
